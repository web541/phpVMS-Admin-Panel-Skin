		...
		foreach($allsettings as $setting) {
			...
			switch($setting->name) {
				...
				case 'CURRENT_SKIN':
					...
					echo '<SELECT class="form-control" name="CURRENT_SKIN">';
				...
				case 'DEFAULT_GROUP':
					...
					echo '<select class="form-control" name="DEFAULT_GROUP">';
				...
				default:
					...
					if($setting->value == 'true' || $setting->value == 'false') {
						...
						echo '<select class="form-control" name="' . $setting->name . '" onChange="showChanged();" >
								<option value="true" '. $sel_true . '>Enabled</option>
								<option value="false" ' . $sel_false . '>Disabled</option>
							  </select>';
					} else {
						echo '<input type="text" class="form-control" name="'.$setting->name.'" value="'.$setting->value.'" />';
					}
